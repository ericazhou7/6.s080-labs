...
def worker(task_queue, out_queue):
    try:
      ...
      while not task_queue.empty():
          tmp_category, tmp_filename = task_queue.get()
          # print("ok", tmp_category, tmp_filename)
          scrape_html(out_queue, tmp_category, tmp_filename)
    except queue.Empty:
        log('Done scraping!')
...
def main_task(urls_df, output_file, n_workers, redownload_html):
    ...
    for i in range(n_workers):
    	p = multiprocessing.Process(target=worker(task_queue, out_queue))
    	processes.append(p)
    	p.start()
    ...
    try:
        while True:
            ...
            csv_rows.append(out_queue.get(block=True, timeout=1))
    except queue.Empty:
        log('Done!')

    end_time = time.perf_counter()
    print("num workers %d time taken %f" % (n_workers, (end_time - start_time)*1000 ))
